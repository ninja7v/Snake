...
if ((x=2) and (i<>2) and (i<>largeurmax-1) and (j<>2) and (j<>hauteurmax-1)) then tab1[i,j]:=2 else tab1[i,j]:=1;
end;
end
else if lvl=0 then
begin
...
begin
if ((tab1[posx,posy-1]=2) or (tab1[posx,posy-1]=0)) then victoire:=false;
...
sens:=haut;
	...
	begin
	if ((tab1[posx-1,posy]=2) or (tab1[posx-1,posy]=0)) then victoire:=false;
...
for i:=1 to maxse do
begin
if (tabscore.namescore[l,i]<>'user') then write(i,'- ',tabscore.namescore[l,i],' : ',tabscore.scorenbr[l,i],' / ');
end;
...
{
var	tab1:array [1..largeurmax,1..hauteurmax] of integer;
	tab2:array [2..100,2..100] of integer;
}
var tab1:tabt1;
	tab2:tabt2;
